...
export const aboutContent = {
  label: 'О нас',
  attribution: 'А8А9 токеномика.рф',
  quote:
  ...
  stats: [
    { value: '6 недель', label: 'средний срок разработки рабочей токеномики' },
    { value: '$180M+', label: 'получили проекты от инвесторов до TGE' },
    {
      value: 'С 2017 года',
      label: 'создаем токеномики для проектов в разных сегментах рынка',
    },
    {
      value: '100% проектов',
      label:
        'на наших моделях, где токен встроен в продукт, а не существует только для торговли',
    },
  ],
} as const
...
export const methodologyContent = {
  ariaLabel: 'Методология А8А9',
  label: 'Методология рейтинга',
  headlineSegments: [
    { text: 'Когда растёт ' },
    { text: 'продукт', accent: true },
    { text: ',\nза ним обязан расти ' },
    { text: 'токен', accent: true },
  ],
  product: {
    label: 'Продукт',
    caption: 'Пользователи · объёмы · выручка',
  },
  token: {
    label: 'Токен',
    caption: 'Спрос подтянут по дизайну',
    topNote: 'Токен ↑ -->',
  },
  linkage: {
  ...
  bodyRest:
    ' — ядро нашего рейтинга. Высокая связка — токен выигрывает вместе с продуктом. Низкая связка — токен остаётся позади, пока бизнес растёт. Именно здесь большинство токенов тихо ломается после TGE.',
  marquee: [
    'Захват ценности',
  ...
  signals: [
    { id: 'value-capture', title: 'Захват ценности', line: 'Доходит ли выручка до токена?' },
  ...
  footnote:
    '+ устойчивость токеномики · фундаментал · управление и контроль · безопасность · рыночный слой — оцениваются так же и собираются в один результат.',
  faq: [
    {
      question: 'Что такое Token Product Linkage?',
      answer:
        'Token Product Linkage показывает, создаёт ли рост продукта реальную экономическую ценность для токена. Он отвечает на простой вопрос: если растут пользователи, выручка, объёмы или активность — получает ли токен спрос, полезность, комиссии, байбеки, сжигания или более сильный захват ценности? Сильный токен не просто торгуется — он связан с системой, которая создаёт ценность.',
    },
    {
      question: 'Как на самом деле работает рейтинг А8А9?',
      answer:
        'Мы оцениваем токен-экономику по фиксированному набору блоков — Token Product Linkage в ядре, плюс устойчивость токеномики, фундаментал, управление и контроль, безопасность и рыночный слой — каждый по одной и той же рубрике, а затем сводим в один балл с цветовой шкалой и кратким разбором ключевых рисков. Мы публикуем, что и почему измеряем, и показываем это в наших [публичных аудитах](/audits).',
    },
  ],
} as const
...
export const homeMeta = {
  title: 'А8А9 токеномика.рф — Токен-экономики, которые усиливают бизнес',
  description:
    'Мы превращаем токены из разового способа привлечь деньги в работающие экономические механики. Они встроены в продукт и операционную логику, поэтому ценность формируется за счет спроса и использования, а не спекуляций.',
  ogTitle: 'А8А9 токеномика.рф — Токен-экономики, которые усиливают бизнес',
  ogDescription:
    'Стратегический консалтинг, дизайн токеномики и техническая экспертиза для Web3 проектов.',
} as const
...
export const servicesMeta = {
  ...
  ogDescription: 'Стратегический консалтинг, проектирование и аудит токеномики.',
} as const
...
export const tokenomicsTestScreens = [
  {
    ...
    title: 'Базовая модель',
    description: 'Задайте объем токенов, цену, циркуляцию и аллокации, чтобы быстро увидеть базовую картину по токеномике',
  },
  {
    src: '/img/miniapp-2.png',
    alt: 'Allocation — распределение токенов',
    title: 'Аллокация',
    description: 'Распределите токены между командой, инвесторами, комьюнити и экосистемой — наглядно и с контролем процентов',
  },
  {
    src: '/img/miniapp-3.png',
    alt: 'Vesting — настройка разлоков',
    title: 'Вестинг и разлоки',
    description: 'Настройте разлоки по каждому пулу, посмотрите графики анлоков, возможные всплески и общее состояние модели',
  },
  {
    src: '/img/miniapp-4.png',
    alt: 'Scoring — итоговая оценка',
    title: 'Итоговая оценка',
    description: 'Получите срез модели с кратким выводом о рисках и качестве токеномики, графиком вестинга и рекомендациями',
  },
] as const

export const tokenomicsTestAriaLabel = 'Тест токеномики' as const
